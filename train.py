...
SFAnet = model.SFA(opt)
Cls = model.classifier(opt)
print(netG)
print(discriminator)
print(SFAnet)
...
optimizerS = optim.Adam(SFAnet.parameters(), lr=0.0001, betas=(opt.beta1, 0.999))
...
for start_step in range(0, opt.nepoch):
    ...
    for iter_d in range(5):
        ...
        mR, discTrue = discriminator(input_resv)
        ...
        mF, discFalse = discriminator(fake.detach())
        ...
        for i in range(data.ntrain_class):
            ...
            if sample_idx.numel() == 0:
                ...
                center_loss1 += (G_sample_cls.mean(dim=0) - cent[i]).pow(
                    2).sum().sqrt()
        ...
        for i in range(data.ntrain_class):
            ...
            if sample_idx.numel() == 0:
                ...
                center_loss2 += (G_sample_cls.mean(dim=0) - cent[i]).pow(
                    2).sum().sqrt()

        gradient_penalty = calc_gradient_penalty(discriminator, input_res, fake.data, input_att)
        D_cost = (center_loss1 + center_loss2) * opt.center_weight + criticD_fake - criticD_real + gradient_penalty
        # outz_real = mR.data[0].norm()
        # real_ins_contras_loss2 = crUtil.new_contras2(mR, input_attv, input_label, opt.d, opt.t2) * opt.lema * 10
        real_ins_contras_loss = contras_criterion(F.normalize(mR, dim=1), input_label)  # + real_ins_contras_loss2
        # outz_fake = mF.data[0].norm()
        # Dfake_ins_contras_loss2 = crUtil.new_contras2(mF, input_attv, input_label, opt.d, opt.t2) * opt.lema
        RC_loss = F.cross_entropy(Cls(mR), input_label)
        fake_ins_contras_loss = contras_criterion(F.normalize(mF, dim=1), input_label)
        D_cost += (real_ins_contras_loss + fake_ins_contras_loss) * opt.cr + RC_loss * opt.cls_weight
    ...
    noisev = Variable(noise)
    fake = netG(noisev, input_attv)
    # a_r = SFAnet(input_resv)
    # R_loss = (input_attv - a_g).pow(2).sum().sqrt() + (input_attv - a_r).pow(2).sum().sqrt()
    mF, discTrue = discriminator(fake)
    mR, discFalse = discriminator(input_resv)
    a_g = SFAnet(mF)
    a_r = SFAnet(mR)
    criticG_fake = discFalse.mean()
    ...
    for i in range(data.ntrain_class):
        ...
        if sample_idx.numel() == 0:
            ...
            center_loss += (G_sample_cls.mean(dim=0) - cent[i]).pow(2).sum().sqrt()
    ...
    FC_loss = F.cross_entropy(Cls(mF), input_label)
    # Gfake_ins_contras_loss2 = crUtil.new_contras2(torch.cat((mF, mR), dim=0), torch.cat((input_attv, input_attv), dim=0) , torch.cat((input_label, input_label), dim=0), opt.d, opt.t2) * opt.lema
    fake_ins_contras_loss = contras_criterion(F.normalize(torch.cat((mF, mR), dim=0)),
                                              torch.cat((input_label, input_label), dim=0))
    errG = center_loss * opt.center_weight + fake_ins_contras_loss * opt.cr - criticG_fake + R_loss * opt.reg + FC_loss * opt.cls_weight
    errG.backward()
    optimizerG.step()
    ...
    log_text = 'Iter-{}; errD: {:.3f} errG:{:.3f} gp:{:.3f} center_loss: {:.3f}, c1:{:.3f},c2:{:.3f}, rc:{:.3f} fc:{:.3f}, R_loss:{:.3f} RC:{:.3f}, FC:{:.3f}  ' \
        .format(start_step, D_cost.item(), errG.item(), gradient_penalty.item(), center_loss.item(),
                center_loss1.item(), center_loss2.item(), real_ins_contras_loss.item(),
                fake_ins_contras_loss.item(), R_loss.item(), RC_loss.item(), FC_loss.item())
    print(log_text)
    # qq=time.time() - start
    # if start_step!=0 and start_step<6:
    #     cc+=qq
    # print("time used:", qq , cc)
    # if start_step==5: print("time avg:", cc/5)
    ...
    if start_step != 0 and start_step % opt.test_epoch == 0 and start_step > opt.begin_step:
        ...
        if opt.use_classify == True:
            cls = classifier.CLASSIFIER(train_X, train_Y, data, opt.nclass_all, True, discriminator, 0.001, 0.5,
                                        opt.cls_epoch, opt.syn_num, True)
            acc_seen = cls.acc_seen
            acc_unseen = cls.acc_unseen

        else:
            train_z, _ = discriminator(train_z)
            test_z_seen, _ = discriminator(test_z_seen)
            test_z_unseen = data.test_unseen_feature.cuda()
            test_z_unseen, _ = discriminator(test_z_unseen)
